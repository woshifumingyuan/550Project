...
bool readOptions(int argc, char** argv, std::vector<Robot>& robots, std::vector<bool>& valid, bool debug)
{
    if (argc < 2)
    {
        ...
        std::cout << "    Square robot with side length=1.0 at the origin with orientation=pi/2 (invalid): s 0 0 0 1.57079 1.0" << std::endl;
        return false;
    }
    ...
    while(true)
    {
        ...
        // Reading in the rest of the configuration
        // Some of these values depend on type
        bool v;
        fin >> v;
        valid.push_back(v);
...
int main(int argc, char** argv)
{
    ...
    if (readOptions(argc, argv, robots, valid, debug))
    {
        ...
        else
        {
            ...
            std::cout << robots.size() - wrong.size() << " / " << robots.size() << " configurations classified correctly [" << pct*100.0 << "%]" << std::endl;

            if (wrong.size())
            {
                std::cout << "The following configurations were classified incorrectly (starting at zero): " << std::endl;
